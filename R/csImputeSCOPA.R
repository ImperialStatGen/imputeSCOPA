...
require('ranger');
require('Rcpp');
sourceCpp('Rcpp/csample.cpp');
require('tictoc');

csImputeUnivariate <- function(x) {
  stopifnot(is.atomic(x), length(x) >= 1L)
  ok <- !is.na(x)
  if (any(!ok)) {
    x[!ok] <- csample_num(x[ok], sum(!ok), replace = TRUE)
  }
  cat("\nUNIV\n");
  print(x[!ok]);
  x
...
csImputeSCOPA <- function(data, maxiter = 10L, pmm.k = 0L, seed = NULL, verbose = 1, ...) {
  ...
  while (crit && j <= maxiter) {
    ...
    for (v in visit.seq) {
      ...
      if (length(completed) == 0L) {
      	 data[[v]] <- csImputeUnivariate(data[[v]])
      } else {
        ndata = data[!v.na, union(v, completed)]
	if (verbose >= 3) {
		cat("\nVisit: ", visit.seq, " Removed: ", which(v.na, arr.ind=TRUE), " \n: ", which(is.na(ndata), arr.ind=TRUE), "\n")
		print(union(v, completed))
	}
        fit <- ranger(formula = reformulate(completed, response = v), 
                      data = ndata, #data[!v.na, union(v, completed)],
		      seed = seed,
                      ...)
